...
import com.agus.java.controller.entitySwagger.LogoutEntity;
...
public class LogoutServiceController {
	...
	public static final String restUrlAddress = ServiceConstants.BASE_REST_URL
			+ ServiceConstants.LOGOUT_SERVICE;
	private static JSONObject boOutput;
	static ServiceOutput serviceOutput = new ServiceOutput();

	@Operation(summary = "Logout API", description = " API untuk logout ", security = { @SecurityRequirement(name = "bearer-key") }, tags = { "logout" })
	...
	public ResponseEntity<Collection<ServiceOutput>> callService(
			@Parameter(required = true, schema = @Schema(implementation = LogoutEntity.class)) @Valid @RequestBody LogoutEntity inputUser)
		...
		finalServiceOutput = commandProcessingService.processAndLogCommand(
				...
				BoConstants.Logout, MasterGeneralConstants.DATAPROCESS,
				requestJSON);
