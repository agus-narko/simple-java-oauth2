...
public class ActivationRoleServiceController {

    @Autowired
    BoInterceptor commandProcessingService;

    private static Map<BigInteger, ServiceOutput> finalServiceOutput;
    public static final String restUrlAddress = ServiceConstants.BASE_REST_URL + ServiceConstants.ACTIVATION_ROLE_SERVICE;
    static ServiceOutput serviceOutput = new ServiceOutput();

    @Operation(summary = "Activaion Role API",
			description = " API untuk menonaktifkan atau mengaktifkan role ",
    ...
    public ResponseEntity<Collection<ServiceOutput>> callService(
        ...
        finalServiceOutput = commandProcessingService.processAndLogCommand(
                ...
                restUrlAddress, BoConstants.ActivationRoleById, MasterGeneralConstants.DATAPROCESS,
                requestJSON);
